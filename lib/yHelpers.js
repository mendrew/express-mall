...
async function getYItem(product, index) {
  ...
  const yUrl = getYQuery(encodeURI(product.name))
  ...
  const productTitleIndex = getProductTitleIndex(itemTitles, product.name)
...
function getProductTitleIndex(titleList, title) {
  ...
  const cleanTitleList = titleList
...
function getTitleMatchIndexByFuseJs(titleList, title) {
  ...
  const matchedTitleIndex = _.get(result, '[0]') && 0
